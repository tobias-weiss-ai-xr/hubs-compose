...
- **Durable repo anchors (2026-08-27):** the fixes now also live in this repo so a client rebuild keeps them — `services/hubs/static-server.py` (canonical, with the slug rewrite), `services/hubs/src/utils/configs.js` (branded `fallbackImages` + chemie-lernen default `theme`, applied in production, not just dev), `services/hubs/public/manifest.webmanifest` (Chemie Lernen Hubs name + green `theme_color` + correct favicon sizes), and `services/hubs/webpack.config.js` (`cache: { type: "filesystem" }` for faster incremental/CI builds). NOTE: the live `hubs-client` image is currently produced by an external CI build (not `dockerfiles/hubs.Dockerfile`, which is a stub); to deploy these repo changes, rebuild the client from this repo (or patch the CI to COPY `services/hubs/static-server.py` + build with the updated `configs.js`/`manifest.webmanifest`).
